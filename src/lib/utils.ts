 ...
 */
export function countUniqueTechnologies(projects: any[]): number {
  const allTechnologies = new Set<string>();
  
  projects.forEach(project => {
    // Count technologies from detailedTechnologies object
    if (project.detailedTechnologies) {
      Object.values(project.detailedTechnologies).forEach((techCategory: any) => {
        if (Array.isArray(techCategory)) {
          techCategory.forEach((tech: any) => {
    ...
    // Fallback to simple technologies array if available
 ...
 */
export function countUniqueCollaborators(projects: any[]): number {
  const allCollaborators = new Set<string>();
  
  projects.forEach(project => {
    // Count partners from projectDetails
    if (project.projectDetails?.partners) {
      project.projectDetails.partners.forEach((partner: any) => {
    ...
    if (project.projectDetails?.softwareEngineers) {
      project.projectDetails.softwareEngineers.forEach((engineer: any) => {
...
export function extractTechnologiesFromDetailed(detailedTechnologies?: { [key: string]: Array<string | { name: string; url: string }> }): string[] {
...
export function getAllTechnologiesFromProjects(projects: any[]): { 
  ...
  projects.forEach(project => {
    ...
    if (project.detailedTechnologies) {
      Object.entries(project.detailedTechnologies).forEach(([category, techs]: [string, any]) => {
    ...
    // Fallback to simple technologies array if it exists
    if (project.technologies) {
      project.technologies.forEach((tech: string) => {
        ...
        if (!categorizedTechnologies.other.includes(tech)) {
          categorizedTechnologies.other.push(tech);
        }
      });
    }
  });
...
export function getAllCategoriesFromProjects(projects: any[]): string[] {
